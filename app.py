...
def check_research_folder():
    """Check if research_papers folder exists and contains PDFs"""
    research_dir = Path("research_papers")
    if not research_dir.exists():
        return False, "research_papers directory does not exist"
    
    pdf_files = list(research_dir.glob("*.pdf"))
    if not pdf_files:
        return False, "No PDF files found in research_papers directory"
...
def create_vector_embedding():
    ...
    with st.spinner("Processing documents... This might take a while ⏳"):
        try:
            ...
            # Limit to first 50 documents if there are more
            docs_to_process = docs[:50] if len(docs) > 50 else docs
            st.session_state.processed_docs = len(docs_to_process)
            ...
            return True, f"Successfully processed {len(docs_to_process)} documents into {len(st.session_state.final_documents)} chunks"
...
def main():
    ...
    folder_exists, folder_message = check_research_folder()
    if not folder_exists:
        st.error(folder_message)
        st.info("Please create a 'research_papers' directory and add your PDF files.")
        return
    ...
    with st.sidebar:
        ...
        st.markdown(
            """
            <div class="info-box">
            1. Click "Process Documents" to create embeddings from your PDFs. 
            2. Enter your question about the research papers. 
        ...
        if "vectors" in st.session_state:
            st.success(f"✅ Documents processed: {st.session_state.processed_docs}/{st.session_state.total_docs}")
    ...
    with col1:
        ...
        st.write(f"Found PDF files in research_papers directory: {folder_message}")
    
    with col2:
        if st.button("Process Documents", key="process_docs"):
            success, message = create_vector_embedding()
            if success:
                st.success(message)
            else:
                st.error(message)
    ...
    if st.button("Submit Question", key="submit_query"):
        if "vectors" not in st.session_state:
            st.warning("Please process the documents first by clicking 'Process Documents'.")
